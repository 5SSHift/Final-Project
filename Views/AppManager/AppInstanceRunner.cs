...
namespace Wpf.Views.AppManager
{
    ...
    public sealed class AppInstanceRunner
    {
            ...
            dispatcher.BeginInvoke(async () =>
            {
                ...
                if (!SessionStore.WasExplicitlyLoggedOut())
                {
                    ...
                    try
                    {
                        ...
                        if (deviceOk)
                        {
                            // Verifică preferința utilizatorului ÎNAINTE de auto-login
                            if (LoginPreferenceService.RequiresManualLogin(userId))
                            {
                                // Userul a ales autentificare manuală — mergem la LoginWindow
        ...
        internal static void ConfigureServices(ServiceCollection svc)
        {
            ...
            svc.AddSingleton<SettingsViewModel>(sp => new SettingsViewModel(sp.GetRequiredService<AuthService>()));
